...
class InventoryTableViewCell: UITableViewCell {
    ...
    private func setupUI() {
        ...
        let stack = UIStackView(arrangedSubviews: [itemNameLabel, locationLabel, categoryLabel, quantityLabel, timeLabel, buttonsStackView])
        stack.axis = .vertical
        stack.spacing = 6
        stack.translatesAutoresizingMaskIntoConstraints = false
    ...
    func configure(with item: Inventory,
        ...
        // Quantity badge color coding
        if item.quantity == 0 {
            quantityLabel.backgroundColor = .systemRed
        } else if item.quantity < 5 {
            quantityLabel.backgroundColor = .systemOrange
        } else {
            quantityLabel.backgroundColor = .systemGreen
        }
    }
